"""ManiMux policy plugins for the MolmoAct2 Bimanual YAM checkpoint."""
...
class MolmoActHttpPolicyModel:
    ...
    def __init__(self, config: PolicyConfig) -> None:
        ...
        self._normalization_tag = _string_option(
            config.options,
            "normalization_tag",
            "yam_dual_molmoact2",
        )
    ...
    def reset(self, session_id: str) -> None:
        ...
        try:
            ...
            raise RuntimeError("MolmoAct health check returned invalid JSON") from exc
    ...
    def infer(self, request: InferenceRequest) -> object:
        ...
        if not isinstance(decoded, dict) or "actions" not in decoded:
            raise ValueError("MolmoAct response must contain actions")
...
class MolmoActYamAdapter:
    """Translate canonical YAM snapshots and raw MolmoAct matrices."""
    ...
    def build_observation(self, snapshot: ObservationSnapshot) -> ObservationSnapshot:
        ...
        if missing:
            raise ValueError(f"MolmoAct YAM adapter is missing cameras: {missing}")
        return snapshot
    ...
    def decode_action(self, raw: object, context: ActionContext) -> ActionChunk:
        ...
        if actions.ndim != 2 or actions.shape[1] != expected_dim:
            raise ValueError(
                f"MolmoAct actions must have shape (horizon, {expected_dim}), got {actions.shape}"
            )
        if not actions.shape[0] or not np.isfinite(actions).all():
            raise ValueError("MolmoAct actions must be non-empty and finite")
        ...
        for name in self._group_order:
            ...
            start = end
        return ActionChunk(
            plan_id=f"molmoact-{context.request_seq}-{uuid.uuid4().hex[:8]}",
    ...
    def validate(self, robot: RobotConfig, policy: PolicyConfig) -> None:
        ...
        if tuple(robot.group_dims) != self._group_order:
            raise ValueError(
                "MolmoAct YAM requires robot groups in order "
                f"{list(self._group_order)}, got {list(robot.group_dims)}"
            )
        if any(robot.group_dims[name] != 7 for name in self._group_order):
            raise ValueError("MolmoAct YAM requires two 7-value arm+gripper groups")


def build_model(config: PolicyConfig) -> MolmoActHttpPolicyModel:
    return MolmoActHttpPolicyModel(config)


def build_adapter(robot: RobotConfig, policy: PolicyConfig) -> MolmoActYamAdapter:
    return MolmoActYamAdapter(robot, policy)
