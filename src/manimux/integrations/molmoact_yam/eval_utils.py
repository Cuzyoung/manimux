...
class EvalRolloutSaver:
    ...
    def flush(self) -> None:
        ...
        with h5py.File(h5_path, "w") as f:
            ...
            f.attrs["camera_names"] = np.array(cam_names_stripped, dtype=h5py.string_dtype())
            f.create_dataset("state", data=states, compression="gzip", compression_opts=4)
            f.create_dataset("next_state", data=next_states, compression="gzip", compression_opts=4)
...
class LiveCameraView:
    ...
    * ``obs`` (no ``pub_endpoint``): legacy path — cv2 runs on the calling
    ...
    def _build_panes(
        ...
        for name, label in zip(self.PUB_CAM_NAMES, self.OBS_LABELS, strict=True):
            img = frames.get(name)
            if img is None:
    ...
    def _update_obs_mode(
        ...
        self._ensure_window()
        panes: list[np.ndarray] = []
        for obs_key, label in zip(self.OBS_KEYS, self.OBS_LABELS, strict=True):
            rgb = obs.get(obs_key)
            if rgb is None:
                continue
...
def prompt_label() -> str | None:
    ...
    while True:
        text = (
            input("Label rollout (y = success / n = failure / Enter = keep as eval): ")
            .strip()
            .lower()
        )
        if text == "y":
