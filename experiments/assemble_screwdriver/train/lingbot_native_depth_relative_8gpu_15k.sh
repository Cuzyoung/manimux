...
CONFIG=${CODE_ROOT}/experiments/assemble_screwdriver/train/lingbot_native_depth_relative.yaml
...
RUN_NAME=${LINGBOT_RUN_NAME:-assemble-screwdriver-lingbot-native-depth-relative-8xh100-b64-15k-20260829-v1}
...
PATH="${VENV}/bin:${PATH}" "${VENV}/bin/python" "${POLICY_ROOT}/prepare_bundle.py" \
  ...
  --norm-stats "${STATS}" \
  --robot-config "${POLICY_ROOT}/robot_configs/${ROBOT_NAME}.yaml" \
  --native-hz 30 \
  --action-horizon 50
